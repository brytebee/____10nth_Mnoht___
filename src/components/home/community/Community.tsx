  ...
  return (
    <section className='mt-10 w-full pb-20 md:mt-20 md:pb-32'>
      <div className='flex flex-col gap-5 mb-10'>
        <div className={`w-[320px] lg:w-[600px] lg:items-center md:w-[550px] mx-auto flex items-center gap-2 ${isSmallMobile && 'w-[95%]'}`}>
          <Image src={AcademicIcon} alt="academic icon" />
          <h2 className='text-center text-xl md:text-3xl font-bold text-body'>Join Our Community of Students</h2>
          <Image src={AcademicIcon} alt="academic icon" />
        </div>
        <p className={`w-[347px] md:w-[700px] lg:w-[878px] mx-auto text-center font-light ${isSmallMobile && 'w-[95%]'}`}>Embarking on a learning journey with our online tech academy is an investment in your future. Empower yourself with the skills and knowledge needed to thrive in the ever-evolving tech industry. Join our community, learn from industry experts, and unlock your potential today. Get ready to shape the future of technology!</p>
      </div>
      {/* <MobileCarousel> */}
      ...
      {/* </MobileCarousel> */}
      <div className='hidden md:grid grid-cols-2 mt-9 lg:w-10/12 md:w-full mx-auto bg-inherit lg:pl-20 md:pl-10 pb-20 pt-16' style={{ boxShadow: '0px 9px 100px 17px rgba(0, 0, 0, 0.10)', borderRadius: '13px' }}>
        {['1', '2', '3', '4'].map((testimony, index) => (
          <div key={index}
            className={`relative lg:w-[340px] md:w-[300px] h-[150px] rounded-[21px] bg-testimonial lg:pt-11 md:pt-6 pl-10 pb-6 pr-3 ${(index === 0 || index === 2) && 'mt-20'} ${index === 0 && 'lg:ml-[80px] lg:mb-16 md:ml-5'} ${index === 1 && 'lg:ml-[60px] md:ml-4'} ${(index === 0 || index === 1) && 'gap-8'}`}
          >
            <Image src={TestimonialImg} alt="testimonial image" className='absolute -top-9 -left-6 md:w-[60px] md:h-[60px] lg:w-[84px] lg:h-[84px]' />
            <Image src={RightQuote} alt="right quote" className='absolute -top-2 right-4 w-[14px] h-[14px]' />
            <p className='font-light text-sm'>Lorem ipsum dolor sit amet consectetur. Ornare sed mattis nunc turpis enim. In justo molestie in a. Ac aliquam malesuada feugiat vitae pharetra.</p>
          </div>
        ))}
      </div>
      <div className='hidden md:flex lg:flex-row md:flex-col-reverse justify-between w-[90%] mx-auto my-16 bg-inherit'>
        <div className='flex flex-col gap-5 flex-1'>
          <h2 className='md:text-4xl lg:text-3xl text-body font-bold text-center'>Our course catalogue</h2>
          ...
          <Link href='#' className='pl-48 flex pt-2 font-semibold text-xl text-hero mb-7'>See all <Image src={ChevronRight} alt='chevron right' /></Link>
        </div>
        <Image src={WorkingMan} alt='man working in office' className='h-[600px] flex-1' />
      </div>
      ...
      <div className='mt-8 px-9 flex flex-col items-center gap-5 md:flex-row md:gap-16 lg:pl-32'>
        <Image src={InstructorImg} alt="instructor image" className='md:w-[503px]' />
        <div className='flex flex-col gap-3 items-center md:items-start'>
          <h3 className='text-center text-body text-xl font-bold md:text-3xl'>Become an instructor</h3>
          <p className='w-[317px] md:w-[362px] font-light text-base mx-auto text-center md:text-left'>Instructors from around the world teach millions of students on techverse, we provide the tools and skills to teach what you love.</p>
